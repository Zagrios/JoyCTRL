...
pub fn get_vk_enigo() -> MutexGuard<'static, Enigo> {
    ENIGO
        .get_or_init(|| Mutex::new(Enigo::new(&Settings::default()).unwrap()))
        .lock()
        .unwrap()
}

pub fn press_key(key: &str, enigo: &mut Enigo) -> Result<(), Box<dyn std::error::Error>> {
    let key = get_enigo_key_from_str(key).ok_or("Invalid key")?;
    ...
    KEYS_TO_RELEASE.write().unwrap().insert(key);
...
pub fn release_all_keys(enigo: &mut Enigo) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for key in KEYS_TO_RELEASE.read()?.iter() {
        enigo.key(*key, Direction::Release)?;
        KEYS_TO_RELEASE.write().unwrap().remove(key);
    }
...
pub fn toogle_vk_window(app: &AppHandle) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(keyboard_window) = keyboard_window {
        return keyboard_window.close().map_err(|e| e.into());
    }
